...
const InboxMainOne = () => {
    ...
    const editHandler = (item) => {
        ...
        if (
            ...
            item.status.toLowerCase() === "not seen"
        ) {
            axios
                .put(`http://127.0.0.1:8000/api/selection-terms/${item.id}`, {
                    status: "seen",
                })
                .then(() => {
                    fetchData(); // Reload data to show updated status in table
                })
                .catch((error) => {
