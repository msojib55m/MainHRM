...
const LeaveHoliday = () => {
    ...
    const [OpenHoliday, setHoliday] = useState(false);
    ...
    useEffect(() => {
        const fetchHolidays = async () => {
            try {
                ...
                setHolidays(response.data); // Set the holidays data
            } catch (error) {
    ...
    // Handle form field change and calculate total days
    const handleChange = (e) => {
        const { name, value } = e.target;
        let updatedForm = { ...formData, [name]: value };

        // Calculate total days if both from_date and end_date are provided
        if (updatedForm.from_date && updatedForm.end_date) {
            const start = new Date(updatedForm.from_date);
            const end = new Date(updatedForm.end_date);
            const timeDiff = end - start;
            updatedForm.total_days =
                timeDiff >= 0 ? timeDiff / (1000 * 60 * 60 * 24) + 1 : 0;
        }

        setFormData(updatedForm);
    };

    // Handle form submission and send data to Laravel
    const handleSubmit = async (e) => {
        e.preventDefault(); // Prevent default form submission

        try {
            // Sending form data to Laravel API
            const response = await axios.post(
                "http://127.0.0.1:8000/api/holidays", // Adjust URL if needed
                formData
            );
            ...
            setHoliday(false); // Close the modal after successful submission
        } catch (error) {
    ...
    // data pass to mysql now
    // ডাটা আপডেট করা হচ্ছে এবং সাথে mysql পরিবতন হচ্ছে
    //  সাচ করার জন্য এটি ব্যবহার করতে হচে
    const [searchQuery, setSearchQuery] = useState("");
    const [entries, setEntries] = useState(10);
    const handleSearch = (event) => {
        setSearchQuery(event.target.value);
    };
    const searchedHolidays = holidays
        .filter((holiday) =>
    ...
    const handleUpdate = async () => {
        ...
        try {
            const response = await fetch(
                `http://127.0.0.1:8000/api/holidaysUpdata/${editHoliday.id}`,
                {
                    method: "PUT",
                    headers: {
            ...
            if (response.ok) {
                ...
                // আপডেট হওয়া ডাটা State-এ সেট করুন
                setHolidays(
                    holidays.map((holiday) =>
                        holiday.id === editHoliday.id
                            ? { ...holiday, ...updatedData }
                            : holiday
                    )
                );
    ...
    // ডাটা এখান থেকে laravel  পাঠাব এবং ডিলেক্ট করার জন্য
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this holiday?")) {
            try {
                const response = await fetch(
                    `http://127.0.0.1:8000/api/holidaysDelete/${id}`,
                    {
                        method: "DELETE",
                        headers: {
                            "Content-Type": "application/json",
                ...
                if (response.ok) {
                    ...
                    // Remove the deleted holiday from state
                    setHolidays(
                                                    ...
                                                    <div className="space-y-4 ml-4">
                                                        ...
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <input
                                                                ...
                                                                className="w-full h-10 border-none bg-gray-200 rounded-md p-2 focus:ring-0 outline-none"
                                                                readOnly
                                                                value={
                                                                    formData.total_days
                                                                }
                            ...
                            <div class="flex justify-between items-center">
                                <div class="mt-[20px]">
                                    <label class="text-sm font-medium">
                                        ...
                                        <select
                                            name="example_length"
                                            class="custom-select custom-select-sm form-control form-control-sm w-[40px] h-[35px] p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 appearance-none"
                                            value={entries}
                                ...
                                <div class="mr-[10px] mt-[20px]">
                                    <div class="flex items-center justify-between w-[250px]">
                                        ...
                                        <div>
                                            <input
                                                type="search"
                                                class="w-full h-[30px] p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500"
                                                placeholder="Search..."
                                                value={searchQuery}
                                ...
                                <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-gray-800 bg-opacity-50">
                                    <div className="bg-white p-6 rounded-lg shadow-lg w-96">
                                        ...
                                        <input
                                            type="text"
                                            name="holiday_name"
                                            value={updatedData.holiday_name}
